...
// This is a fork of OpenZeppelin Contracts (last updated v5.0.0) (metatx/ERC2771Forwarder.sol)
// to modify the _execute function to return the revert messages form nested contract calls
// The bubble up revert happens only for the single execute function and not for the batch execute function
 ...
 */
contract ERC2771Forwarder is EIP712, HashNonce {
  ...
  /**
   * @dev Util function to _return the hash of the fully encoded EIP712 message
   */
   ...
   * 
   * TODO: This function has been changed to a non-view method, along with all underlying internal calls, because of a bug with `sload` OPCODE
   * See below for more explanation in `isTrustedByTarget`
   */
   ...
   * out of gas. Will revert if the request is invalid or the innerCall reverts, in this case the nonce is not consumed.
   *
   * Requirements:
   *
  ...
  function execute(ForwardRequestData calldata request) public payable virtual {
    ...
    (bool callSuccess, bytes memory returndata) = _execute(request, true);

    if (!callSuccess) {
      // Revert with the error message from the external contract
      if (returndata.length > 0) {
        assembly {
          revert(add(32, returndata), mload(returndata))
        }
      } else {
        revert("InnerCall failed without revert reason");
      }
    }
  }
  ...
  function _recoverForwardRequestSigner(
    ForwardRequestData calldata request
  ) internal virtual returns (bool, address) {
  ...
  function _execute(
  ...
  ) internal virtual returns (bool callSuccess, bytes memory returndata) {
    ...
    if (isTrustedForwarder && signerMatch && active) {
      ...
      _useNonce(signer, request.nonce);
      ...
      emit ExecutedForwardRequest(signer, request.nonce, callSuccess);
